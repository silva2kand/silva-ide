...
import { ocrImage } from "../../agents/ravan-vision-lab";
...
export class MessageRouter {
  ...
  private getChannelForAdapter(adapter: ChannelAdapter): Channel | undefined {
    const mappedChannelId = this.adapterChannelIds.get(adapter);
    if (mappedChannelId) {
      return this.channelRepo.findById(mappedChannelId);
    }
    return this.channelRepo.findByType(adapter.type);
  }
  ...
  registerAdapter(adapter: ChannelAdapter, channelId?: string): void {
    const resolvedChannelId =
      channelId || this.channelRepo.findByType(adapter.type)?.id;
    if (resolvedChannelId) {
      this.adaptersByChannelId.set(resolvedChannelId, adapter);
      this.adapterChannelIds.set(adapter, resolvedChannelId);
    }
  ...
  /**
   * Get adapter bound to a specific channel ID
   */
  getAdapterForChannel(channelId: string): ChannelAdapter | undefined {
    return this.adaptersByChannelId.get(channelId);
  }

  /**
   * Unregister adapter bound to a specific channel ID
   */
  unregisterAdapterForChannel(channelId: string): void {
    const adapter = this.adaptersByChannelId.get(channelId);
    if (!adapter) return;

    this.adaptersByChannelId.delete(channelId);
    this.adapterChannelIds.delete(adapter);

    if (this.adapters.get(adapter.type) === adapter) {
      const replacement = Array.from(this.adaptersByChannelId.values()).find(
  ...
  getAllAdapters(): ChannelAdapter[] {
    const seen = new Set<ChannelAdapter>();
    const all: ChannelAdapter[] = [];
    for (const adapter of this.adaptersByChannelId.values()) {
  ...
  async connectAll(): Promise<void> {
    ...
    for (const channel of enabledChannels) {
      const adapter =
        this.getAdapterForChannel(channel.id) ||
  ...
  async sendMessage(channelType: ChannelType, message: OutgoingMessage): Promise<string> {
    ...
    const typedAdapters = Array.from(this.adaptersByChannelId.values()).filter(
      (candidate) => candidate.type === channelType,
    );
    const adapter =
      typedAdapters.find((candidate) => candidate.status === "connected") ||
    ...
    try {
      const channel = this.getChannelForAdapter(adapter) || this.channelRepo.findByType(channelType);
      if (channel) {
   ...
   */
  async sendMessageToChannel(channelId: string, message: OutgoingMessage): Promise<string> {
    ...
    if (adapter.status !== "connected") {
      throw new Error(`Adapter ${channel.type} (${channelId}) is not connected`);
    }

    this.cleanupIdempotencyCache();
    const cacheKey = this.getIdempotencyCacheKey(channel.type as ChannelType, message);
    if (cacheKey) {
      const existing = this.sentIdempotencyKeys.get(cacheKey);
      if (existing) {
        return existing.messageId;
    ...
    try {
      ...
      this.emitEvent({
        type: "message:sent",
        channel: channel.type as ChannelType,
        timestamp: new Date(),
        data: { chatId: message.chatId, messageId },
      });
    } catch (logError) {
      console.warn(`[Router] Failed to log outgoing message (${channel.type}:${channel.id}):`, logError);
    }
  ...
  private async forwardToDesktopApp(
    ...
    if (savedAttachments.length > 0) {
      if (adapter.type === "email") {
        const baseSourceId = String(message.messageId || "").trim() || `email-${Date.now()}`;
        void (async () => {
          for (const att of savedAttachments) {
            const mime = String(att.mimeType || "").toLowerCase();
            const isImage = att.type === "image" || mime.startsWith("image/");
            const isPdf = mime === "application/pdf" || att.absPath.toLowerCase().endsWith(".pdf");
            if (!isImage && !isPdf) continue;
            try {
              const text = await ocrImage(att.absPath);
              await saveOcrDocument(text, "email_attachment", `${baseSourceId}:${att.relPath}`);
            } catch {
            }
          }
        })();
  ...
  async finalizeDraftStreamForTask(taskId: string, finalText: string): Promise<void> {
    ...
    try {
      ...
      try {
        const channel = this.getChannelForAdapter(pending.adapter);
  ...
  private async maybeSendTaskFeedbackControls(opts: {
    ...
    try {
      const resolvedChannel = this.getChannelForAdapter(adapter);
      const messageId = resolvedChannel
        ? await this.sendMessageToChannel(resolvedChannel.id, {
            chatId: opts.pending.chatId,
