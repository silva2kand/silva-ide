...
export class ChannelGateway {
    ...
    const config =
      protocol === "loom"
        ? {
            ...
            historicalSyncStartDate: options?.historicalSyncStartDate,
            historicalSyncBatchSize: options?.historicalSyncBatchSize,
  ...
  updateChannel(channelId: string, updates: Partial<Channel>): void {
    ...
    const adapter = this.router.getAdapterForChannel(channel.id);
  ...
  async enableChannel(channelId: string): Promise<void> {
    ...
    let adapter = this.router.getAdapterForChannel(channel.id);
    if (!adapter) {
  ...
  async enableWhatsAppWithQRForwarding(channelId: string): Promise<void> {
    ...
    // Create and register adapter if not already done
    let adapter = this.router.getAdapterForChannel(channel.id) as WhatsAppAdapter | undefined;
    if (!adapter) {
  ...
  async removeChannel(channelId: string): Promise<void> {
    ...
    if (channel.type === "whatsapp") {
      const adapter = this.router.getAdapterForChannel(channel.id) as WhatsAppAdapter | undefined;
      if (adapter) {
        await adapter.logout();
      } else {
    ...
    this.router.unregisterAdapterForChannel(channelId);
  }
  ...
  async sendMessageToSession(
    ...
    return this.router.sendMessageToChannel(session.channelId, {
    ...
    switch (channel.type) {
      ...
      case "email":
        ...
        return createEmailAdapter({
          ...
          historicalSyncStartDate: channel.config.historicalSyncStartDate as string | undefined,
          historicalSyncBatchSize: channel.config.historicalSyncBatchSize as number | undefined,
          responsePrefix: channel.config.responsePrefix as string | undefined,
