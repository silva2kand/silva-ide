...
import { startCctvPosHeartbeatHook, CctvPosFrame } from "./hooks/cctv-pos-heartbeat";
...
<body>
  ...
  app.whenReady().then(async () => {
    ...
    try {
      ...
      try {
        const baseWorkspacePath = heartbeatDeps.getDefaultWorkspacePath();
        const frameProvider = async (): Promise<CctvPosFrame[]> => {
          const frames: CctvPosFrame[] = [];
          const base = baseWorkspacePath;
          if (!base) return frames;
          const candidates = [
            { dir: path.join(base, ".cowork", "inbox", "cctv"), type: "cctv" as const },
            { dir: path.join(base, ".cowork", "inbox", "pos"), type: "pos" as const },
          ];
          const imageExtRe = /\.(png|jpe?g|webp|bmp|tiff?)$/i;
          for (const { dir, type } of candidates) {
            try {
              ...
              for (const item of items) {
                ...
                if (!imageExtRe.test(filePath)) continue;
                const stat = await fs.stat(filePath);
                frames.push({
                  id: `${type}:${item.name}:${Math.floor(stat.mtimeMs)}`,
                  path: filePath,
                  type,
                });
              }
          ...
          frames.sort((a, b) => b.id.localeCompare(a.id));
          return frames.slice(0, 20);
        };
        startCctvPosHeartbeatHook(frameProvider, 60 * 60 * 1000);
      } catch {
        // ignore
      }
