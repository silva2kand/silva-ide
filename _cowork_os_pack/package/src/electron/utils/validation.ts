...
const createEmailAddExtras = (): z.ZodRawShape => ({
  ...
  emailHistoricalSyncStartDate: z
    .string()
    .regex(/^\d{4}-\d{2}-\d{2}$/)
    .optional(),
  emailHistoricalSyncBatchSize: z.number().int().min(1).max(500).optional(),
...
const createEmailUpdateExtras = (): z.ZodRawShape => ({
  ...
  historicalSyncStartDate: z
    .string()
    .regex(/^\d{4}-\d{2}-\d{2}$/)
    .optional(),
  historicalSyncBatchSize: z.number().int().min(1).max(500).optional(),
