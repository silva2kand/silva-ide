...
export class MemoryTierService {
  ...
  private static getPromotionMinReferenceCount(
    fromTier: MemoryTier,
    toTier: MemoryTier,
    fallback: number,
  ): number {
    return (
      this.PROMOTION_RULES.find((r) => r.fromTier === fromTier && r.toTier === toTier)
        ?.minReferenceCount ?? fallback
    );
  }
  ...
  static runPromotionPass(db: Database.Database): PromotionPassResult {
    ...
    try {
      ...
      const shortToMediumThreshold = this.getPromotionMinReferenceCount("short", "medium", 3);
      const shortToMedium = db
        ...
        .run(shortToMediumThreshold);
      promoted += shortToMedium.changes;

      // Promote medium → long
      const mediumToLongThreshold = this.getPromotionMinReferenceCount("medium", "long", 10);
