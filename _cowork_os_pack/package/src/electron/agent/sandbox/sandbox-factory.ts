...
export interface SandboxOptions {
  ...
  /** Allowed temporary file paths (e.g., for Ollama/Claude interactions) */
  allowedTempPaths?: string[];
...
export interface ISandbox {
   ...
   * Returns the executable, arguments, and a cleanup function
   */
  wrapCommand?(command: string, options?: SandboxOptions): Promise<{ executable: string; args: string[]; cleanup: () => void }>;
...
export class NoSandbox implements ISandbox {
  ...
  async wrapCommand(command: string, options: SandboxOptions = {}): Promise<{ executable: string; args: string[]; cleanup: () => void }> {
