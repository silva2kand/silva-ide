...
export class ShellTools {
  ...
  async runCommand(
    ...
    let spawnArgs = getShellArgs(resolvedShell, effectiveCommand);
    let spawnEnv = safeEnv;
    let cleanupSandbox = () => {};

    if (sandbox.wrapCommand) {
      try {
        const wrapResult = await sandbox.wrapCommand(effectiveCommand, {
          cwd,
          timeout,
          envPassthrough: Object.keys(safeEnv),
          allowedReadPaths: [require("os").tmpdir(), path.join(require("os").homedir(), ".claude")],
          allowedWritePaths: [require("os").tmpdir(), path.join(require("os").homedir(), ".claude")],
        });
        spawnCommand = wrapResult.executable;
        spawnArgs = wrapResult.args;
        cleanupSandbox = wrapResult.cleanup;
      } catch (error: any) {
        this.daemon.logEvent(this.taskId, "command_output", {
          command,
          type: "error",
          output: `\n[Sandbox wrap error: ${error.message}]\n`,
        });
        return {
          success: false,
          stdout: "",
