...
const LOCAL_OPENAI_COMPATIBLE_PROVIDER_IDS = new Set<LLMProviderType>([
  "openai-compatible",
  "hf-agents",
  "jan",
  "lm-studio",
...
function isLoopbackBaseUrl(baseUrl: string | undefined): boolean {
  const normalized = normalizeModelKey(baseUrl);
...
function shouldAutoCorrectLocalModelSelection(
  providerType: LLMProviderType,
  baseUrl?: string,
): boolean {
  const resolvedProviderType = resolveCustomProviderId(providerType);
  if (!LOCAL_OPENAI_COMPATIBLE_PROVIDER_IDS.has(resolvedProviderType)) {
    return false;
  }

  if (resolvedProviderType === "openai-compatible") {
...
function resolvePreferredLocalModelSelection(
  currentModel: string | undefined,
  cachedModels: CachedModelInfo[] | undefined,
): { modelKey: string; reason?: "missing" | "auto" | "unavailable" } {
  const normalizedModel = normalizeModelKey(currentModel) || "";
  const availableModels = Array.isArray(cachedModels)
    ? cachedModels.filter((entry) => normalizeModelKey(entry.key))
    : [];
  ...
  if (!normalizedModel) {
    return { modelKey: availableModels[0].key, reason: "missing" };
  }

  if (normalizedModel.toLowerCase() === "auto") {
    return { modelKey: availableModels[0].key, reason: "auto" };
  }

  if (availableModels.some((entry) => entry.key === normalizedModel)) {
    return { modelKey: normalizedModel };
  }

  return { modelKey: availableModels[0].key, reason: "unavailable" };
}
...
export class LLMProviderFactory {
  ...
  static resolveTaskModelSelection(
    ...
    const providerBaseUrl =
      providerType === "openai-compatible"
        ...
        : customConfig?.baseUrl || customEntry?.baseUrl;
    if (shouldAutoCorrectLocalModelSelection(providerType, providerBaseUrl)) {
      const availableModels = this.getProviderModelStatus({ ...settings, providerType }).models;
      const correctedModel = resolvePreferredLocalModelSelection(resolvedModelKey, availableModels);
      if (correctedModel.modelKey && correctedModel.modelKey !== resolvedModelKey) {
        const previousModel = normalizeModelKey(resolvedModelKey);
        const reasonSuffix =
          correctedModel.reason === "missing"
            ? "the current selection was empty"
            : correctedModel.reason === "auto"
              ? 'the current selection was set to "auto"'
              : `the current selection "${previousModel}" was not returned by the local endpoint`;
        warnings.push(
          `[LLMProviderFactory] Local provider "${providerType}" auto-corrected model selection to "${correctedModel.modelKey}" because ${reasonSuffix}.`,
        );
        resolvedModelKey = correctedModel.modelKey;
      }
    }
  ...
  private static detectProviderFromSettings(settings: LLMSettings): LLMProviderType | null {
    ...
    if (settings.openaiCompatible?.baseUrl && settings.openaiCompatible?.model) {
  ...
  static getProviderModelStatus(settings: LLMSettings): {
    ...
    if (customEntry) {
      ...
      const cachedModels =
        customConfig?.cachedModels && customConfig.cachedModels.length > 0
          ...
          : ((!isFromAnotherProvider && storedModel) || customEntry.defaultModel || "")
            ? [
                {
                  key: ((!isFromAnotherProvider && storedModel) || customEntry.defaultModel || "") as string,
                  displayName: ((!isFromAnotherProvider && storedModel) || customEntry.defaultModel || "") as string,
      ...
      const rawCurrentModel =
        (!isFromAnotherProvider && storedModel) || customEntry.defaultModel || "";
      const currentModel = shouldAutoCorrectLocalModelSelection(
        resolvedProviderType,
        customConfig?.baseUrl || customEntry.baseUrl,
      )
        ? resolvePreferredLocalModelSelection(rawCurrentModel, cachedModels).modelKey
        : rawCurrentModel;
      return {
        currentModel,
        ...
        const modelList =
          settings.cachedOpenAICompatibleModels && settings.cachedOpenAICompatibleModels.length > 0
            ...
            : settings.openaiCompatible?.model
              ? [
                  {
                    key: settings.openaiCompatible.model,
                    displayName: settings.openaiCompatible.model,
                    description: "OpenAI-compatible model",
                  },
                ]
              : [];
        const rawCurrentModel = settings.openaiCompatible?.model || "";
        const currentModel = shouldAutoCorrectLocalModelSelection(
          "openai-compatible",
          settings.openaiCompatible?.baseUrl,
        )
          ? resolvePreferredLocalModelSelection(rawCurrentModel, modelList).modelKey
          : rawCurrentModel;
        return {
          currentModel,
          models: ensureCurrentModel(modelList, currentModel),
  ...
  static async getCustomProviderModels(
    ...
    if (documentedModels.length > 0 && (resolvedProviderType === "minimax" || resolvedProviderType === "minimax-portal")) {
      ...
      updatedSettings.customProviders = {
        ...updatedSettings.customProviders,
        [resolvedProviderType]: Object.assign(
          {},
