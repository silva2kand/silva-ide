    ...
    if (validated.type === "email") {
      ...
      const channel = await gateway.addEmailChannel(
        ...
        validated.securityMode || "pairing",
        {
          ...
          historicalSyncStartDate: validated.emailHistoricalSyncStartDate,
          historicalSyncBatchSize: validated.emailHistoricalSyncBatchSize,
...
function setupMemoryHandlers(): void {
  ...
  // Test local Piper TTS command
  ipcMain.handle(IPC_CHANNELS.VOICE_TEST_LOCAL_TTS, async () => {
    try {
      const voiceService = getVoiceService();
      return await voiceService.testLocalTtsConnection();
    } catch (error: Any) {
      console.error("[Voice] Failed to test local Piper:", error);
      return { success: false, error: error.message };
    }
  });

  // Test local Whisper STT command
  ipcMain.handle(IPC_CHANNELS.VOICE_TEST_LOCAL_STT, async () => {
    try {
      const voiceService = getVoiceService();
      return await voiceService.testLocalSttConnection();
    } catch (error: Any) {
      console.error("[Voice] Failed to test local Whisper:", error);
      return { success: false, error: error.message };
    }
  });
